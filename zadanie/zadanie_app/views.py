...
def add(request):
    if request.method == "POST":
        if (request.POST['detail'] != "" and request.POST['category'] != ""):
            task = Task(detail=request.POST['detail'], status=Task.INCOMPLETE,
                        category=string.capwords(request.POST['category']))
            task.save()
...
def get_chart_data(request):
    task = Task.objects.values('category').annotate(complete=Count("status", filter=Q(status=Task.COMPLETE)),
                                                    incomplete=Count("status", filter=Q(status=Task.INCOMPLETE)))
